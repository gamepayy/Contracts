...
describe("MerkleMinter", function () {
    ...
    before(async () => {
        ...
        const data = fs.readFileSync('test/integration/entries.txt', 'utf8');

        const acc1Address = acc1.address;
        const acc2Address = acc2.address;
        const acc3Address = acc3.address;
        const tokenAddress = integrationTestToken.address; 
        const zeroAddress = ethers.constants.AddressZero;
        
        let preparedData = data.replace(/acc1\.address/g, acc1Address);
        preparedData = preparedData.replace(/acc2\.address/g, acc2Address);
        preparedData = preparedData.replace(/acc3\.address/g, acc3Address);
        preparedData = preparedData.replace(/integrationTestToken\.address/g, tokenAddress);
        preparedData = preparedData.replace(/ethers.constants.AddressZero/g, zeroAddress);
        
        // Split into lines
        const lines = preparedData.split('],');
        
        // Parse the data
        const merkleData = lines.map((line, index) => {
            // Remove brackets and split into parts
            const parts = line.replace(/[\[\]']+/g,'').split(', ');
        
            if (parts.length !== 3) {
                console.error(`Line ${index + 1} does not contain the expected number of values`);
            }
